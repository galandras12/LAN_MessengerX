...
lmcCrypto::~lmcCrypto(void) {
	...
	for (EVP_CIPHER_CTX_wrapper* ctx : encryptMap)
		delete ctx;
	encryptMap.clear();
	for (EVP_CIPHER_CTX_wrapper* ctx : decryptMap)
		delete ctx;
	decryptMap.clear();
}
...
QByteArray lmcCrypto::generateAES(QString* lpszUserId, QByteArray& pubKey) {
	...
	EVP_CIPHER_CTX_wrapper* ectx = new EVP_CIPHER_CTX_wrapper();
	EVP_EncryptInit_ex(ectx->ptr(), EVP_aes_256_cbc(), NULL, keyIv, keyIv + keyLen);
	delete encryptMap.take(*lpszUserId);
	encryptMap.insert(*lpszUserId, ectx);

	EVP_CIPHER_CTX_wrapper* dctx = new EVP_CIPHER_CTX_wrapper();
	EVP_DecryptInit_ex(dctx->ptr(), EVP_aes_256_cbc(), NULL, keyIv, keyIv + keyLen);
	delete decryptMap.take(*lpszUserId);
...
QByteArray lmcCrypto::encrypt(QString* lpszUserId, QByteArray& clearData) {
	...
	EVP_CIPHER_CTX_wrapper* ctx = encryptMap.value(*lpszUserId);
	if(ctx == NULL) {
		free(outBuffer);
		lmcTrace::write("Error: No encryption context for user");
		return QByteArray();
	}
...
QByteArray lmcCrypto::decrypt(QString* lpszUserId, QByteArray& cipherData) {
	...
	EVP_CIPHER_CTX_wrapper* ctx = decryptMap.value(*lpszUserId);
	if(ctx == NULL) {
		free(outBuffer);
		lmcTrace::write("Error: No decryption context for user");
		return QByteArray();
	}
