...
void MsgStream::readyRead(void) {
	//	Messages are framed on the wire as a 4-byte length header followed by
	//	that many bytes of payload. TCP makes no guarantee that a single
	//	readyRead() call delivers a whole header or a whole payload in one
	//	piece - a header or payload split across multiple reads used to
	//	corrupt inDataLen (reading it from a truncated 4-byte buffer) and
	//	could send this loop into a bad state. lenData/inData now accumulate
	//	bytes across as many readyRead() calls as it takes before anything
	//	is parsed or emitted.
	while(socket->bytesAvailable() > 0) {
		if(!reading) {
			qint64 needed = sizeof(quint32) - lenData.size();
			lenData.append(socket->read(needed));
			if(lenData.size() < (int)sizeof(quint32))
				return;	//	header not fully received yet, wait for more data

			QDataStream stream(lenData);
			stream >> inDataLen;
			lenData.clear();
			inData.clear();
			reading = true;
			...
			qint64 remaining = inDataLen - inData.length();
			QByteArray data = socket->read(remaining);
			if(data.isEmpty())
				return;
			inData.append(data);
			if(inData.length() >= (int)inDataLen) {
				reading = false;
				QByteArray message = inData;
				inData.clear();
				emit messageReceived(&peerId, &peerAddress, message);
			}
		}
	}
}
